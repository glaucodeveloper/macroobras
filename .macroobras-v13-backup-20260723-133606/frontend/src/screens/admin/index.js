...
export function adminAccessConfig() {
  return `${pageHeader("Configuração de acesso", "Login da máquina e provedores OAuth do Firebase Authentication.")}
    <div class="grid two access-config-grid">
      ${card("Máquina autorizada pelo GitHub", `<label>Repositório autorizado<input value="glaucodeveloper/macroobras" readonly></label><label>Token da máquina<input type="password" placeholder="Substituir token GitHub" data-machine-github-token></label><button class="btn" data-message="save-machine-token">Atualizar autorização</button>`)}
      ${card("Firebase Authentication", `<label>Configuração JSON<textarea data-firebase-config placeholder='{"apiKey":"…","authDomain":"…","projectId":"…"}'></textarea></label><div class="auth-provider-row"><span>Google OAuth</span><b>Firebase</b></div><div class="auth-provider-row"><span>GitHub OAuth</span><b>Firebase</b></div><button class="btn" data-message="save-firebase-config">Salvar configuração</button>`)}
    </div>`;
}

export function adminLogin() {
  return `<main class="admin-login-page"><section class="admin-login-card"><div class="login-brand"><strong>MacroObras</strong><span>Acesso administrativo</span></div><h1>Entrar na estação</h1><p>Use a conta autorizada no Firebase ou o token GitHub desta máquina.</p><button class="auth-button google" data-message="firebase-login" data-provider="google">Continuar com Google</button><button class="auth-button github" data-message="firebase-login" data-provider="github">Continuar com GitHub</button><div class="login-divider"><span>ou</span></div><label>Token GitHub da máquina<input type="password" data-admin-github-token placeholder="github_pat_…"></label><button class="btn full" data-message="admin-token-login">Entrar com token</button></section></main>`;
}
