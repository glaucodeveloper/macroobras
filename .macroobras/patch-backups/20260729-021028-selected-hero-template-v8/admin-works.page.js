...
function worksVisitPanel(work) {
  if (!state.worksVisitOpen || !work) return "";
  return `<aside class="works-visit-drawer" aria-label="Planejamento de visita">
    <header><div><small>Visita integrada ao mapa de obras</small><h2>${esc(work.name)}</h2></div><button data-message="close-works-visit" aria-label="Fechar planejamento">×</button></header>
    <section class="works-visit-summary"><div><small>Distância</small><strong data-route-distance>0 km</strong></div><div><small>Deslocamento</small><strong data-route-travel>0 min</strong></div><div><small>Visitas</small><strong data-route-visits>0 min</strong></div><div><small>Origem</small><strong data-route-source>Aguardando traçado</strong></div></section>
    <div class="visit-map-wrap"><div class="route-draw-status" data-route-draw-status>Segure o marcador e mova o cursor para calcular a rota.</div><div class="google-map" data-google-map="visits"><div class="map-loading"><span></span><strong>Carregando rota de visita</strong><small>Google Maps e Google Routes</small></div></div><div class="visit-map-commands"><button data-visit-map-command="undo">Desfazer trecho</button><button data-visit-map-command="clear">Limpar rota</button><button class="btn" data-message="generate-visit-plan">Gerar calendário</button></div><div class="route-live-order" data-route-live-order><small>Nenhuma parada conectada</small></div></div>
  </aside>`;
}
