...
function consolidateSelectedWork(scope, map) {
  ...
  hero.classList.add(
    ...
    "works-selected-sidebar-source-v7",
  );

  hero.dataset.worksSelectedSidebarMounted = "v7";
  ...
  if (dashboard && dashboard !== hero) {
    ...
    body.className =
      "works-selected-hero-dashboard-content";
    body.appendChild(fragment);
    hero.appendChild(body);
    dashboard.remove();
  }

  const commandCenter = scope.querySelector(
    ".works-command-center",
  );

  const mapStage = commandCenter?.querySelector(
    ":scope > .works-map-stage",
  );
  ...
  commandCenter.classList.add(
    "works-command-center--selected-v7",
  );

  commandCenter.dataset.selectedHeroMounted = "v7";

  commandCenter.querySelectorAll(
    ":scope > [data-works-selected-sidebar-mounted]",
  ).forEach((mounted) => {
    if (mounted !== hero) mounted.remove();
  });

  /*
   * Movimento estrutural definitivo:
