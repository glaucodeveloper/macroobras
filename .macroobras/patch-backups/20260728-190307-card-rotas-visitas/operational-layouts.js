...
function hydrateWorksVisitToolbar(root) {
  ...
  toolbar.innerHTML = `
    <header class="works-visit-toolbar-heading">
      <small>Rotas de visita</small>
      <strong>Planejamento de visitas</strong>
    ...
    <div class="works-visit-primary-actions">
      <button
        type="button"
        class="btn"
        data-message="new-works-visit-route"
      >
        Novo trajeto
      </button>
    ...
    <div
      class="route-live-order works-route-live-order"
      data-route-live-order
      aria-label="Ordem das paradas"
    >
      ${liveOrderMarkup()}
    </div>

    <div class="works-route-metrics" data-works-route-metrics>
      <span>
        <small>Distância</small>
        <strong data-route-distance>${esc(routeDistance())}</strong>
      </span>
      <span>
        <small>Deslocamento</small>
        <strong data-route-travel>${esc(routeTravel())}</strong>
      </span>
      <span>
        <small>Paradas</small>
        <strong data-route-visits>${routeStops().reduce((sum, stop) => sum + Number(stop.durationMinutes || 0), 0)} min</strong>
      </span>
      <span>
        <small>Fonte</small>
        <strong data-route-source>${state.visitRoute?.segments?.length ? "Google Routes" : "Aguardando traçado"}</strong>
      </span>
    </div>
    ...
    <div class="works-visit-toolbar-actions">
      ...
      <button type="button" class="btn ghost full" data-message="navigate" data-route="admin-visits">
        Abrir planejador completo
      </button>
    </div>

    <footer class="works-visit-drag-help">
      <b>Como criar o trajeto</b>
      <span>Pressione uma obra por 0,6 s, arraste pela direção desejada e solte no próximo ponto.</span>
    </footer>
  `;
}
