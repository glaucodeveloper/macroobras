...
function renderMapsAuthError(message) {
  document
    .querySelectorAll("[data-google-map]")
    .forEach((element) => {
      element.dataset.mapAuthFailed = "true";
      element.innerHTML = `
        <div class="map-auth-error" role="alert">
          <span>!</span>
          <strong>Google Maps indisponível</strong>
...
function installMapsAuthFailureHandler() {
  window.gm_authFailure = () => {
    const message =
      "A chave foi recusada pelo Google Maps. "
      + "Verifique faturamento, APIs ativadas "
      + "e os endereços autorizados da aplicação.";

    console.error(
      "Google Maps authentication failure. "
      + "Abra o console para consultar o código "
      + "de erro retornado pela API."
    );

    renderMapsAuthError(message);
  };
}
...
function loadGoogleMaps() {
  ...
  if (!apiKey) {
    const message =
      "Configure GOOGLE_MAPS_API_KEY em "
      + "~/.config/macroobras/google-maps.env.";
...
function infoContent(work) {
  return `<div class="gm-info gm-static-context">
    <small>${esc(work.code)}</small>
    <strong>${esc(work.name)}</strong>
    <span>${esc(work.address)}</span>
    <div><b>${esc(work.status)}</b><b>${Number(work.progress || 0)}% executado</b></div>
    <nav>
      <button type="button" data-map-work-route="admin-work-overview" data-map-work-id="${esc(work.id)}">Visão geral</button>
      <button type="button" data-map-work-route="admin-work-diary" data-map-work-id="${esc(work.id)}">Diário</button>
      <button type="button" data-map-work-visit="${esc(work.id)}">Planejar visita</button>
    </nav>
  </div>`;
}
...
function bindStaticInfoActions(info) {
  google.maps.event.addListenerOnce(info, "domready", () => {
    ...
    document.querySelectorAll("[data-map-work-visit]").forEach((button) => {
      button.addEventListener("click", () => {
        document.dispatchEvent(new CustomEvent("macroobras:open-work-visit", {
          detail: { workId: button.dataset.mapWorkVisit },
        }));
        info.close();
      });
    });
...
function inventoryCityFromAddress(value) {
...
function inventoryMapLocations() {
  const materials = state.inventoryMaterials || [];
  const works = availableWorks.map((work) => {
    const city = inventoryCityFromAddress(work.address);
    return {
      id: `work:${work.id}`,
      kind: "work",
      workId: work.id,
      ...
      materials: materials.filter((material) => material.workId
        ...
        : normalizeInventoryText(material.city) === normalizeInventoryText(city)),
    };
  });
  const knownCities = new Set(works.map((item) => normalizeInventoryText(item.city)));
  const extraCities = [...new Set(materials.map((item) => item.city).filter((city) => city && !knownCities.has(normalizeInventoryText(city))))];
  return [
    ...works,
    ...extraCities.map((city) => ({
      ...
      coordinates: null,
      materials: materials.filter((material) => normalizeInventoryText(material.city) === normalizeInventoryText(city)),
    })),
  ];
}

function inventoryMarkerContent(location) {
  const element = document.createElement("div");
  element.className = `gm-route-point gm-static-point ${location.kind === "work" ? "gm-inventory-work" : "gm-inventory-depot"}`;
  element.setAttribute("role", "button");
  element.tabIndex = 0;
  element.innerHTML = `<span class="gm-route-dot">${location.kind === "work" ? "O" : "D"}</span><span class="gm-route-label">${esc(location.code)}</span>`;
  element.title = `${location.name}. Clique para abrir o inventário.`;
  return element;
}

function inventoryInfoContent(location) {
  const quantity = location.materials.reduce((sum, material) => sum + Number(material.availableQuantity || 0), 0);
  const represented = location.materials.reduce((sum, material) => sum + Number(material.availableQuantity || 0) * Number(material.averageUnitValue || 0), 0);
  return `<div class="gm-info gm-static-context">
    <small>${esc(location.code)}</small><strong>${esc(location.name)}</strong><span>${esc(location.address)}</span>
    <div><b>${location.materials.length} material(is)</b><b>${quantity.toLocaleString("pt-BR")} unidades</b></div>
    <nav><button type="button" data-map-inventory-location="${esc(location.id)}">Abrir inventário</button></nav>
    <p>Valor representado: ${represented.toLocaleString("pt-BR", { style: "currency", currency: "BRL" })}</p>
  </div>`;
}

async function renderInventoryMap(element) {
  const [{ Map, InfoWindow }, { AdvancedMarkerElement }] = await Promise.all([
    google.maps.importLibrary("maps"),
    google.maps.importLibrary("marker"),
  ]);
  const locations = inventoryMapLocations();
  const fallback = availableWorks[0]?.coordinates || { lat: -12.5, lng: -41.5 };
  const map = new Map(element, mapOptions(fallback, 6));
  const info = new InfoWindow();
  const bounds = new google.maps.LatLngBounds();
  let points = 0;
  ...
  for (const location of locations) {
    if (!location.coordinates) {
      try {
        const result = await geocode(location.address);
        location.coordinates = cloneCoordinates(result.location);
      } catch {
        continue;
      }
    }
    const marker = new AdvancedMarkerElement({
      ...
      title: location.name,
      content: inventoryMarkerContent(location),
      gmpClickable: true,
    });
    bounds.extend(location.coordinates);
    points += 1;
    marker.addEventListener("gmp-click", () => {
      info.setContent(inventoryInfoContent(location));
      info.open({ map, anchor: marker });
      google.maps.event.addListenerOnce(info, "domready", () => {
        document.querySelector(`[data-map-inventory-location="${CSS.escape(location.id)}"]`)?.addEventListener("click", () => {
          document.dispatchEvent(new CustomEvent("macroobras:select-inventory-location", {
  ...
  if (points > 1) map.fitBounds(bounds, 70);
    ...
    const location = locations.find((item) => item.coordinates);
    map.setCenter(location.coordinates);
    map.setZoom(14);
  }
  const selected = locations.find((location) => location.id === element.dataset.selectedLocationId);
  if (selected?.coordinates) map.panTo(selected.coordinates);
  element.__macroMap = map;
}
...
function dispatchRouteDrawing(active, route = null) {
  document.dispatchEvent(
    new CustomEvent("macroobras:visit-route-drawing", {
      detail: {
...
async function renderVisitsMap(element) {
  ...
  const cancelDrawing = (message = "Traçado cancelado.") => {
    dispatchRouteDrawing(false, route);
  ...
  const activateDrawingHold = () => {
    ...
    dispatchRouteDrawing(true, route);
...
export async function hydrateGoogleMaps(root = document) {
  ...
  if (!elements.some((element) => ["visits", "works-visits"].includes(element.dataset.googleMap))) {
    cleanupActiveVisitsMap();
    cleanupActiveVisitsMap = () => {};
  }
  ...
  for (const element of elements) {
    ...
    try {
      ...
      else if (type === "visits" || type === "works-visits") await renderVisitsMap(element);
      else if (type === "inventory") await renderInventoryMap(element);
