...
const steps = [
  ...
  { title: "Acesso do encarregado", caption: "ngrok reconhecido pelo backend" },
...
function collaboratorStep(status) {
  const endpoint = status.publicAppUrl
    || status.collaboratorEndpoint
    || "Aguardando reconhecimento do ngrok pelo backend";

  return installerCard(
    "Acesso público do encarregado",
    "A estação inicia e reconhece o ngrok automaticamente. Esta interface apenas apresenta o estado informado pelo backend.",
    `
      <div class="endpoint-card wide">
        <strong>Endpoint público reconhecido</strong>
        <code>${esc(endpoint)}</code>
        <p>O endereço permanece disponível enquanto o processo responsável pelo túnel estiver ativo.</p>
      </div>
      <div class="installer-status-line ${status.endpointReady ? "ok" : ""}">
        <b>${status.endpointReady ? "✓" : "3"}</b>
        <span>
          <strong>${status.endpointReady ? "Endpoint ngrok reconhecido" : "Aguardando o backend"}</strong>
          <small>${esc(status.endpointMessage || "O backend está verificando o túnel ngrok automaticamente.")}</small>
        </span>
      </div>
    `
  );
