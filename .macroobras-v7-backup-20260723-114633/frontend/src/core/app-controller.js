...
function generateVisitPlan() {
  ...
  const workIds = route.workIds || [];
  if (workIds.length < 2 || !(route.segments || []).length) {
    updateState({ toast: "Conecte pelo menos duas obras no mapa antes de gerar o calendário." });
    return;
  }

  const travelMinutes = Math.round(Number(route.totalDurationMillis || 0) / 60000);
  const visitMinutes = workIds.reduce((sum, workId) => sum + Number(state.visitDurations[workId] || 60), 0);
  const segmentByOrigin = new Map((route.segments || []).map((segment) => [segment.fromId, segment]));
  let cursor = 8 * 60;
  const schedule = workIds.map((workId, index) => {
    const duration = Number(state.visitDurations[workId] || 60);
    const start = minutesToClock(cursor);
    const end = minutesToClock(cursor + duration);
    cursor += duration;
    const nextSegment = segmentByOrigin.get(workId);
    if (index < workIds.length - 1) cursor += Math.round(Number(nextSegment?.durationMillis || 0) / 60000);
    return { workId, start, end, durationMinutes: duration };
  });
