...
function desktopFrame({ state, screen }) {
  ...
  return `
    <header class="topbar maximus-topbar">
      ...
      <button class="top-icon" aria-label="Notificações">⌁</button>
    ...
    <div class="desktop-layout ${children.length ? "has-sidebar" : "no-sidebar"}">
      ${children.length ? `<aside class="sidebar drill-sidebar"><div class="sidebar-title">${isWorkDrill ? "Página da obra" : "Subnavegação"}</div>${children.map((item) => `<button class="${item.route === route ? "active" : ""}" data-message="navigate" data-route="${esc(item.route)}"><span>${esc(item.label)}</span><b>›</b></button>`).join("")}</aside>` : ""}
