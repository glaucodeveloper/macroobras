...
proc ensurePrivateConfigDir() =
  createDir(configDir())
  when defined(posix):
    discard execShellCmd(
      "chmod 700 " & quoteShell(configDir())
    )
...
proc migrateLegacyInstallerConfig() =
  ensurePrivateConfigDir()

  let target = configDir()
  let candidates = @[
    appRootDir() / "config",
    appRootDir().parentDir() / "config",
    parentProjectRoot() / "config",
    parentProjectRoot() / "build" / "desktop" / "config"
  ]
  let filenames = @[
    "machine-access.env",
    "users.json",
  ...
  for filename in filenames:
    ...
    for legacyDir in candidates:
      if legacyDir == target:
        continue

      let source = legacyDir / filename
...
proc githubRepositorySlug(): string =
  ...
  if configured.len == 0 or configured == "glaucodeveloper/erp-da-construcao-maximus-empreendimentos":
...
proc verifyGithubToken(token: string): JsonNode =
  ...
  try:
    ...
    writeFile(
      machineAccessPath(),
      "GITHUB_MACHINE_TOKEN=" & token.strip() &
