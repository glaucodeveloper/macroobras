  ...
  canvas.querySelectorAll("[data-diagram-node]").forEach((node) => {
    node.addEventListener("pointerdown", (event) => {
      ...
      const draw = () => {
        frame = 0;
        if (!pending) return;
        const point = clientToWorld(canvas, pending.clientX, pending.clientY);
      ...
      const up = () => {
        ...
        if (dragged) {
          ...
          setTimeout(() => { delete node.dataset.diagramDragSuppressed; }, 0);
        } else delete node.dataset.diagramDragSuppressed;
        dispatchChange(canvas, model);
      };
...
function printDiagramCanvas(canvas) {
  canvas.classList.add("diagram-print-target");
  document.body.classList.add("printing-diagram");
  const cleanup = () => {
    canvas.classList.remove("diagram-print-target");
