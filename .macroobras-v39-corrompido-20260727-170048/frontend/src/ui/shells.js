...
function renderSidebar({ route, isWorkDrill, current, sidebarItems, collapsed, hoverMode }) {
...
function renderSidebar({ route, isWorkDrill, current, sidebarItems, collapsed, hoverMode }) {
  ...
  return `<aside id="carbon-subnavigation" class="sidebar drill-sidebar" aria-label="Subnavegação da seção" data-collapsed="${collapsed}">
    <button class="sidebar-fold-handle" data-message="toggle-sidebar" aria-controls="carbon-subnavigation" aria-expanded="${!collapsed}" aria-label="${hoverMode ? "Fixar subnavegação aberta" : "Recolher subnavegação para abrir ao passar o mouse"}" title="${hoverMode ? "Fixar menu aberto" : "Recolher menu"}">
      <span class="sidebar-fold-corner" aria-hidden="true"></span>
      ${shellIcon(hoverMode ? "chevronRight" : "chevronLeft")}
    </button>
    <aside id="carbon-subnavigation" class="sidebar drill-sidebar" aria-label="Subnavegação da seção" data-collapsed="${collapsed}">
    <header class="sidebar-subnav-header">
      <div><small>${isWorkDrill ? "Página da obra" : "Subnavegação"}</small><strong>${isWorkDrill ? "Obra selecionada" : esc(current?.label || "Seção")}</strong></div>
      <button class="sidebar-collapse-control" data-message="toggle-sidebar" aria-label="${hoverMode ? "Fixar subnavegação" : "Usar expansão ao passar o mouse"}" title="${hoverMode ? "Fixar subnavegação" : "Usar expansão por hover"}">${shellIcon(hoverMode ? "pin" : "chevronLeft")}</button>
    </header>
    <nav>
      ${sidebarItems.map((item) => `<button class="${item.route === route ? "active" : ""}" data-message="navigate" data-route="${esc(item.route)}" ${item.route === route ? 'aria-current="page"' : ""}><span>${esc(item.label)}</span>${shellIcon("chevronRight")}</button>`).join("")}
    </nav>
  </aside>`;
}
