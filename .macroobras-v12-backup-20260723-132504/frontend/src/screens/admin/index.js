...
function diagramSeed(work) {
  ...
  const nodes = work.items.map((item, index) => ({
    ...
    date: `2026-${String(8 + Math.floor(index / 6)).padStart(2, "0")}-${String(3 + (index * 3) % 25).padStart(2, "0")}`,
...
export function adminWorkDiary() {
  ...
  const diarySeed = {
    nodes: seed.nodes.map((node, index) => ({
      ...
      description: `${node.date.split("-").reverse().join("/")} · item do cronograma`,
    })),
  ...
  return `${workHeader(work, "Diário de obras", "Área gráfica dos itens do Cronograma e da sequência registrada em campo.")}
    ${card("Fluxo registrado da obra", `<div class="interactive-diagram diary-diagram" data-interactive-diagram="diary-${esc(work.id)}" data-fixed-edges="true" data-hide-node-dates="true"><svg data-diagram-svg></svg><div data-node-layer></div><script type="application/json">${JSON.stringify(diarySeed)}</script></div>`, "interactive-diagram-card diary-graphic-only")}`;
}
...
export function adminVisits() {
  ...
  return `${pageHeader("Visitas", "Segure um ponto de obra, mova o mouse e solte em qualquer cidade ou ponto da estrada para criar uma parada.", `<button class="btn" data-message="generate-visit-plan">Gerar calendário</button>`)}
    ...
    ${card("Paradas do traçado", stops.length ? `<div class="current-route-list">${stops.map((stop, index) => `<article><b>${index + 1}</b><span><strong>${esc(stop.name)}</strong><small>${esc(stop.address || "")}</small></span><label>Parada<input type="number" min="0" step="15" value="${Number(stop.durationMinutes || 60)}" data-visit-duration="${esc(stop.id)}"><small>min</small></label></article>`).join("")}</div>` : `<div class="route-empty-state"><b>●</b><strong>Inicie em um ponto de obra</strong><span>Segure o ponto, mova o mouse e acompanhe o caminho pelas estradas. Ao soltar, informe o tempo da nova parada.</span></div>`)}
...
export function adminSettings() {
  return `${pageHeader("Administração", "Configuração dos acessos, publicação mobile e modelos gráficos.")}
    <section class="settings-grid"><button data-message="navigate" data-route="admin-mobile-platform"><b>◎</b><strong>Plataforma mobile</strong><span>Publicação segura pelo ngrok para o mestre.</span></button><button data-message="navigate" data-route="admin-access"><b>AF</b><strong>Acessos do mestre</strong><span>Email, CPF e obra permitida.</span></button><button data-message="navigate" data-route="admin-diagram-library"><b>↗</b><strong>Modelos de diagramas</strong><span>Cronograma, compras e RH.</span></button></section>`;
}

export function adminMobilePlatform() {
  const platform = state.collaboratorStatus || {};
  const address = platform.publicAppUrl || platform.localAppUrl || "Aguardando endereço público do ngrok";
  const online = Boolean(platform.publicAppUrl || platform.available);
  return `${pageHeader("Plataforma mobile", "O ngrok permanece ativo automaticamente enquanto a estação MacroObras estiver aberta.")}
    <section class="mobile-address-only ${online ? "online" : "waiting"}">
      <small>Endereço de acesso do mestre de obras</small>
      ${online || String(address).startsWith("http") ? `<a href="${esc(address)}" target="_blank" rel="noreferrer">${esc(address)}</a>` : `<strong>${esc(address)}</strong>`}
    </section>`;
}
...
export function adminDiagramLibrary() {
  const operational = {
    nodes: [
      ...
      { id: "sys-field", kind: "Campo", title: "Mestre de obras mobile", description: "Cronograma, diário e comprovação de entrega", x: 750, y: 330 },
  ...
  const purchases = {
    nodes: [
      ...
      { id: "buy-delivery", kind: "Campo", title: "Entrega", description: "Foto enviada pelo mestre", x: 1030, y: 90 },
  ...
  return `${pageHeader("Modelos de diagramas", "Áreas gráficas para reconhecer os fluxos de funcionamento e reutilizar a lógica de quadros e conexões.")}
    ${card("Fluxo operacional do sistema", `<div class="interactive-diagram system-flow-diagram" data-interactive-diagram="system-operational" data-fixed-edges="true" data-hide-node-dates="true"><svg data-diagram-svg></svg><div data-node-layer></div><script type="application/json">${JSON.stringify(operational)}</script></div>`, "interactive-diagram-card")}
    ${card("Fluxo de compra e comprovação", `<div class="interactive-diagram system-flow-diagram" data-interactive-diagram="system-purchases" data-fixed-edges="true" data-hide-node-dates="true"><svg data-diagram-svg></svg><div data-node-layer></div><script type="application/json">${JSON.stringify(purchases)}</script></div>`, "interactive-diagram-card diagram-section-gap")}`;
}
