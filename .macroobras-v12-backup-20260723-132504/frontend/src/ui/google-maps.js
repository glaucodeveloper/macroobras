...
async function renderVisitsMap(element) {
  ...
  const flushRoadPreview = async () => {
    ...
    try {
      ...
      const roadEnd = result.path.at(-1) || request.destination;
      const liveCursor = drawing.currentCoordinates || request.destination;
      cursorGuide?.setPath([roadEnd, liveCursor]);
      cursorGuide?.setOptions({ strokeOpacity: 0.22, strokeWeight: 2 });
      setDrawStatus(wrap, `Rota acompanhando o mouse: ${(result.distanceMeters / 1000).toFixed(1)} km · ${durationText(result.durationMillis)}`, "drawing");
    } catch (error) {
  ...
  const pointerMove = (event) => {
    ...
    if (!previewLayer) cursorGuide?.setPath([drawing.origin.coordinates, cursor]);
