...
function collaboratorBadge(status) {
  ...
  const label = safe.available ? "Mobile online" : safe.lanActive ? "Mobile na rede" : safe.localActive ? "Mobile local" : "Plataforma mobile";
  const detail = safe.publicAppUrl || safe.lanAppUrl || safe.localAppUrl || safe.message;
  return `<button class="collab-badge ${safe.available ? "online" : safe.lanActive || safe.localActive ? "local" : ""}" data-message="navigate" data-route="admin-mobile-platform"><strong>${esc(label)}</strong><span>${esc(detail || safe.message)}</span></button>`;
}
