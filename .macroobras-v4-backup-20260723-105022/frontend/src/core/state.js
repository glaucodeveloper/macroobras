...
function initialSelectedWorkId() {
  if (!isTwaSurface()) return sessionStorage.getItem("macroobras.selectedWorkId") || availableWorks[0]?.id || "";
...
export const state = {
  ...
  workPageTab: sessionStorage.getItem("macroobras.workPageTab") || "overview",
...
export function selectedPurchaseFlow() {
  return state.purchaseFlows.find((flow) => flow.id === state.selectedPurchaseFlowId) || state.purchaseFlows[0] || null;
}
...
export function updateState(patch) {
  ...
  if (isInstallerSurface()) {
    ...
    sessionStorage.setItem("macroobras.selectedWorkId", state.selectedWorkId || "");
    sessionStorage.setItem("macroobras.workPageTab", state.workPageTab || "overview");
  }
  renderCallback();
}

export function dependencyMessage() {
  return "Selecione uma obra antes de criar serviços, rotinas, cronograma, compras, entregas ou diário.";
}
