...
export function mobileDay() {
  ...
  return `${mobileHeader("Cronograma do dia", "Gerado pelo planejamento por nós")}<section class="mobile-page"><div class="info">As datas foram definidas nos quadros do cronograma administrativo.</div>${entries.map((entry) => `<button class="timeline ${entry.tone}" data-message="toggle-day-entry" data-entry-id="${esc(entry.id)}"><span>${entry.time}</span><h2>${esc(entry.title)}</h2><p>${esc(work.name)}</p><b>›</b></button>`).join("")}${mobileNav("Hoje")}</section>`;
}
