...
function infoContent(work) {
  return `<div class="gm-info"><small>${work.code}</small><strong>${work.name}</strong><span>${work.address}</span><div><b>${work.status}</b><b>${work.progress}% executado</b></div><button data-gm-open-work="${work.id}">Abrir obra</button></div>`;
}
  ...
  availableWorks.forEach((work, index) => {
    ...
    marker.addListener("click", () => {
      ...
      google.maps.event.addListenerOnce(info, "domready", () => {
        document.querySelector(`[data-gm-open-work="${work.id}"]`)?.addEventListener("click", () => {
