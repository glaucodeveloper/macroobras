...
export function adminMobilePlatform() {
  ...
  const address = platform.publicAppUrl
    || "Aguardando reconhecimento do ngrok pelo backend";
  const online = String(address).startsWith("https://");

  return `${pageHeader(
    "Plataforma mobile",
    "O backend inicia, reconhece e acompanha o endpoint ngrok automaticamente."
  )}
    <section class="mobile-address-only ${online ? "online" : "waiting"}">
      <small>Endereço público do encarregado</small>
      ${online
        ? `<a href="${esc(address)}" target="_blank" rel="noreferrer">${esc(address)}</a>`
        : `<strong>${esc(address)}</strong>`}
      <p>${esc(platform.message || "Aguardando o estado informado pelo backend.")}</p>
    </section>`;
}
