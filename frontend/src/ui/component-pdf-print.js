...
function handlePdfExport(event) {
  ...
  const canvas = (
    window.MacroObrasResolveCanvasForPdf?.(button)
    || (
      target?.matches?.("[data-interactive-diagram]")
        ? target
  ...
  if (
    canvas
    && typeof window.MacroObrasPrintCanvasTableReport
      === "function"
  ) {
    ...
    window.MacroObrasPrintCanvasTableReport(canvas);
    return;
  }
