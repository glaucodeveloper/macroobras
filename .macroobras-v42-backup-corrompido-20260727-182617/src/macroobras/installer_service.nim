...
proc githubRepositorySlug(): string =
  ...
  if configured.len == 0 or
      configured == "glaucodeveloper/erp-da-construcao-maximus-empreendimentos":
    "glaucodeveloper/macroobras"
  else:
    configured
...
proc emailInstaladorValido(
  ...
  if (
    atIndex <= 0
    or atIndex >= normalized.high
  ):
    return false

  let dotIndex =
    normalized.find('.', atIndex + 2)

  result =
    dotIndex > atIndex + 1
    and dotIndex < normalized.high

proc digitoCpfInstalador(
  digits: string;
  length: int
): int =
  var sum = 0
...
proc executeInstallerPayload*(
  ...
  if installationCompleted():
    return %*{
      ...
      "mensagem":
        "A estação já possui um administrador e não executará o instalador novamente."
    }
  ...
  if not machineAuthorized():
    return %*{
      ...
      "status": installerStatusPayload(),
      "mensagem":
        "Autorize a máquina pelo GitHub antes de concluir."
    }
  ...
  if (
    not dirExists(folder)
    or not ftp{"ftpVerified"}.getBool(false)
  ):
    return %*{
      "instalado": false,
      "status": installerStatusPayload(),
      "mensagem":
        "Selecione a pasta e verifique a transmissão FTP."
    }

  let access = machineAccess()
  let adminName = payload{
    "adminName"
  }.getStr(
    access{
      "GITHUB_MACHINE_LOGIN"
    }.getStr("Administrador")
  ).strip()
  let adminEmail = payload{
    "adminEmail"
  }.getStr("").strip().toLowerAscii()
  let adminCpf = payload{
    "adminCpf"
  }.getStr("").strip()
  ...
  if adminName.len < 2:
    return %*{
      ...
      "status": installerStatusPayload(),
      "mensagem":
        "Informe o nome do primeiro administrador."
    }
  ...
  if not cpfInstaladorValido(adminCpf):
    return %*{
      ...
      "status": installerStatusPayload(),
      "mensagem":
        "Informe um CPF administrativo válido."
    }
  ...
  %*{
    ...
    "status": installerStatusPayload(),
    "mensagem":
      "Primeiro administrador criado. A instalação não será exibida nos próximos acessos."
  }
