...
task test, "Compile backend without opening desktop window":
  exec "nim c -d:sslheck --path:../../obra_macroobras_design_types_tests_bundle/src src/app.nim"

task testEmbedNgrok, "Compile-check backend with embedded ngrok for the current platform":
  exec "nim c -d:sslheck -d:macroobrasEmbedNgrok --path:../../obra_macroobras_design_types_tests_bundle/src src/app.nim"
