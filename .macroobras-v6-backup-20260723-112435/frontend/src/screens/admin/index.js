...
export function adminVisits() {
  ...
  const visitMinutes = availableWorks.reduce((sum, work, index) => sum + Number(state.visitDurations[work.id] || 60 + index * 30), 0);
  return `${pageHeader("Visitas", "Trace a rota no Google Maps e defina a duração de cada visita.", `<button class="btn" data-message="generate-visit-plan">Gerar calendário</button>`)}
    <div class="visit-layout"><section><div class="visit-map-wrap">${map("visits")}<div class="visit-duration-panel">${availableWorks.map((work, index) => `<label><span>${index + 1}</span><strong>${esc(work.name)}</strong><input type="number" min="15" step="15" value="${state.visitDurations[work.id] || 60 + index * 30}" data-visit-duration="${esc(work.id)}"><small>min</small></label>`).join("")}</div></div><div class="route-metrics"><span><small>Distância</small><strong data-route-distance>Calculando…</strong></span><span><small>Deslocamento</small><strong data-route-travel>Calculando…</strong></span><span><small>Visitas</small><strong>${Math.floor(visitMinutes / 60)}h ${visitMinutes % 60}min</strong></span><span><small>Fonte</small><strong data-route-source>Google Routes</strong></span></div></section><aside class="saved-routes"><h3>Calendários gerados</h3>${state.visitPlans.map((plan) => `<button class="${selected?.id === plan.id ? "active" : ""}" data-message="select-visit-plan" data-plan-id="${esc(plan.id)}"><strong>${esc(plan.name)}</strong><small>${esc(plan.date)} · ${plan.workIds.length} obras</small><span>${Math.floor((plan.travelMinutes + plan.visitMinutes) / 60)}h ${(plan.travelMinutes + plan.visitMinutes) % 60}min</span></button>`).join("")}</aside></div>
    ${selected ? card("Calendário da rota", `<div class="route-calendar">${selected.workIds.map((id, index) => { const work = workById(id); return `<article><b>${String(index + 1).padStart(2, "0")}</b><span><strong>${esc(work.name)}</strong><small>${esc(work.address)}</small></span><time>${8 + index * 3}:00 — ${10 + index * 3}:00</time></article>`; }).join("")}</div>`) : ""}`;
}
